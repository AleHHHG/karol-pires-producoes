...
class OrderChartService
  ...
  def bar_chart_data
    items = Order.by_period(initial_date, end_date).joins(:project).group('projects.name').count
    { labels: items.keys, values: items.values}
  end

  def pie_chart_data
    items = Order.by_period(initial_date, end_date).select('projects.name as project_name, sum(price) as price').grouped_by_project
